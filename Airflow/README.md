...
**Почему Airflow не подключается к my_postgres кототрый в другом .yaml файле?**
...
Тоесть 5423 это номер канала, по которому клиент (в данном случае мой ПК - приложение dbvear) подключается к сервису.
