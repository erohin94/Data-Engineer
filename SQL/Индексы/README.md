...
**Индексы** — это средство увеличения производительности БД. Это структуры данных, которые ускоряют поиск строк в таблице. Используя индекс, сервер баз данных может находить и извлекать нужные строки гораздо быстрее, чем без него. Можно представить их как оглавление в книге: вместо того чтобы читать всю книгу, ты находишь нужную страницу по оглавлению.
...
Без индекса база делает Full Table Scan — читает всю таблицу.
Если строк 1 млрд, это очень медленно.
Система будет сканировать всю таблицу, строку за строкой, что не очень эффективно.

Если есть индекс по `user_id`, `CREATE INDEX idx_user_id ON users(user_id);`, то сканирование будет быстрее, так как придется опустится на несколько уровней в дереве поиска.
Аналогия с оглавлением в книге, чтобы найти главу, мы смотрим содержание и переходим на нужную страницу, а не листаем все страницы по порядку.
...
Индекс — это ключ + указатель на строку. Пример:
...
Когда выполняется запрос: `WHERE user_id = 3`

1. ищется значение в индексе

2. берётся указатель

3. читается строка из таблицы
   
# Типы индексов

По умолчанию `CREATE INDEX` создаёт индексы-B-tree, эффективные в большинстве случаев. 
...
Сложность поиска если строк: `1 000 000 000`. Высота дерева обычно: 3-4 уровня. Поэтому поиск: `O(log n)` это очень быстро.

## 2. Hash

Хэш-индекс в PostgreSQL — это тип индекса, который использует хэш-функцию для быстрого поиска данных. Он эффективно работает для равенства (поиск по точному совпадению) и часто используется для операций типа `=`. 
Однако, хэш-индексы в PostgreSQL не поддерживают диапазонные запросы (например, `BETWEEN`, `ORDER BY` или `>`, `<`).

**Как работает**

Работает через хеш-функцию. Значение преобразуется в хеш.
...
Создаём HASH индекс по колонке `name`.
...
Как работает HASH индекс. Он хранит `hash(name)`.Пример (упрощённый):
...
Теперь, когда у нас есть хэш-индекс, можно выполнять поиск по имени пользователя. Например, если мы ищем пользователя по имени `Alice`, запрос будет работать быстрее, чем если бы индекса не было:

```
SELECT * FROM users WHERE name = 'Alice';
```

Этот запрос будет использовать созданный хэш-индекс для быстрого нахождения строки, где `name = 'Alice'`. 

PostgreSQL считает hash: `hash('Alice')`.

Находит bucket: `bucket_1`.

Берёт строки из `bucket`.

Хэш-индексы обеспечивают очень быстрые поисковые операции, поскольку они используют хэш-функции, которые представляют значение как числовой хэш.

## Другие виды индексов

**Bitmap Index** - Используется для колонок с маленьким количеством значений. Плохо работает если колонка имеет миллионы уникальных значений.

**Full Text Index** - Используется для поиска текста.

**Composite Index (составной)** - Индекс на несколько колонок.
...
**1. Если в таблице: `1 000 000 000` строк. Индекс может занимать: `20-40%` размера таблицы. Что это значит?**

Смысл вопроса — понимание того, что индекс хранит копию данных.

Например есть таблтца `users`. С колонками `user_id`, `email`, `name`, `address`, `phone`.

Размер строки: `200 байт`

Количество строк: `1 000 000 000`

Размер таблицы: `200 GB`

Индекс по `user_id` хранит: key + pointer (Ключ + указатель), `user_id → pointer`.

Размер: `8 байт` ключ, `8 байт` указатель. Итого: `16 байт`.

Размер индекса `16 bytes × 1 000 000 000 = 16 GB`.

Если несколько индексов. Например: `user_id`, `email`, `phone` три индекса. Размер: `16GB + 30GB + 25GB ≈ 70GB`

Таблица: `200GB`. Индексы: `70GB`. И получается: `~35%`

**2.  Почему нельзя индексировать всё?**

Ответ:

1. индексы занимают много места

2. каждый `INSERT` должен обновлять индекс

3. каждый `UPDATE`обновляет индекс

4. много индексов = медленные записи


В системах типа Hadoop, Spark, Impala часто вообще нет индексов. Используют: `partitioning`, `clustering`, `bucketing`. Это замена индексам в big data.
