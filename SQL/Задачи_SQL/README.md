...
`COUNT(*) AS rank`: Подсчитываем количество строк в каждой группе, что и будет рангом. Тоесть какое количество строк в группе, таким и будет по величине(рангу) число.
...
**Задача — какой результат вернут запросы ниже. Сначала для таблицы 1, затем для таблицы 2**
...
`null = null → NULL (не TRUE, не FALSE — неизвестно)`

`null <> null → NULL`

`123 <> null → NULL (любое сравнение с NULL дает NULL)`

`null is null → TRUE`

Логика: `(NULL OR NULL OR NULL OR TRUE) → в итоге TRUE`

Результат: Вернутся все строки из tab1 (при условии, что tab1 существует).

Разбор условий 2 запрос:

`1 = 1 → TRUE`

Результат: Все строки из tab1. Оба запроса вернут одинаковый набор строк (все строки tab1).

Для второй таблицы оба запроса вернут обе строки, включая строку с `NULL` в name (так как условие `WHERE` не проверяет колонку `name`).
