...
**Работа с диском (как в Hadoop MapReduce):** Вам нужно каждый раз ходить в кладовку (диск) за каждым ингредиентом, потом относить туда же готовый полуфабрикат, чтобы потом снова пойти в кладовку за ним для следующего шага. Это очень медленно из-за постоянных "ходок".
