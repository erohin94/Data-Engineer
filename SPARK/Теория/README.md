...
**RDD** – это распределенная коллекция данных (строки, числа, кортежи, словари и тд), расположенных по нескольким узлам кластера, набор объектов Java или Scala, представляющих данные. RDD работает со структурированными и с неструктурированные данными. Также, как DataFrame и DataSet, RDD не выводит схему загруженных данных и требует от пользователя ее указания.

<img width="756" height="326" alt="image" src="https://github.com/user-attachments/assets/9f6ab153-14df-4de6-8d8f-1bfa138ba0e4" />

Появился начиная с Spark 0 (Low-Level-API). Partitions могут храниться на разных узлах кластера. Отказоустойчивость: RDD можно восстановить при сбое. Работает с любыми данными, RDD не имеет схемы(Spark не знает имён и типов полей). Используется функциональный API: map, filter, reduce. Нет автоматической оптимизации

Пример:

```
rdd = sc.parallelize([("Alice", 25), ("Bob", 30), ("Cathy", 27), ("Dan", 22)], numSlices=2)
rdd.filter(lambda x: x[1] > 26).collect()
#[('Bob', 30), ('Cathy', 27)]

Распределение по узлам:
Кластер:
 ├── Узел 1 → [("Alice", 25), ("Bob", 30)]
 ├── Узел 2 → [("Cathy", 27), ("Dan", 22)]
```

**DataFrame** – это распределенная коллекция данных в виде именованных столбцов, аналогично таблице в реляционной базе данных. DataFrame работает только со структурированными и полуструктурированными данными, организуя информацию по столбцам, как в реляционных таблицах. Это позволяет Spark управлять схемой данных. Это набор типизированных записей, разбитых на блоки. Иными словами — таблица, состоящая из строк и столбцов. Блоки могут обрабатываться на разных нодах кластера.

Появился начиная с Spark 1.3 (Structured API). Имеет явную схему: у каждой колонки есть имя и тип данных. Именованные столбцы (как в SQL). Типизация динамическая: Spark знает типы данных, но компилятор нет. Ошибки (например, опечатки в названии столбца) проявляются только во время выполнения. SQL-подобный API: select, groupBy, filter, join

Оптимизация: Catalyst Optimizer — строит и оптимизирует план выполнения SQL. Tungsten Engine — эффективно использует память и CPU для быстрого выполнения

Пример:

```
data = [("Alice", 25), ("Bob", 30), ("Cathy", 27), ("Dan", 22)]
...
Появился начиная с Spark 1.6 (Structured API). Только структурированные данные. Статическая типизация: типы данных известны уже на этапе компиляции (ошибки ловятся заранее, до запуска кода). Объединяет SQL-подобный и функциональный API. Оптимизация: Catalyst + Tungsten. Python: Dataset отсутствует, используется DataFrame


**Catalyst и Tungsten, что это?**

**Catalyst Optimizer** — умная оптимизация логики запроса: строит план выполнения, переставляет фильтры, объединяет операции для ускорения SQL/DataFrame

**Tungsten Engine** — оптимизация железа и памяти: работа с байтами в памяти, эффективное использование CPU, меньше сборки мусора
...
Теперь это снова обычный `dict`, с которым можно работать.
...
Spark SQL в PySpark — это модуль Apache Spark, который предоставляет интеграцию между обработкой данных с использованием SQL и программированием на Python. Он позволяет выполнять SQL-запросы на данных, хранящихся в Spark, а также предоставляет высокоуровневый интерфейс для работы с данными с использованием DataFrame API. Основные компоненты Spark SQL включают DataFrame и SQL API.

**Основные компоненты Spark SQL в PySpark:**

**1.DataFrame:**

- DataFrame — это распределенная коллекция данных, организованных в виде именованных столбцов. Он аналогичен таблице в реляционной базе данных или DataFrame в pandas.

- DataFrame API предоставляет множество функций для работы с данными, включая фильтрацию, агрегацию, сортировку и объединение.

**2.SQL API:**

- SQL API позволяет выполнять SQL-запросы непосредственно на DataFrame.

- Вы можете зарегистрировать DataFrame как временную таблицу и использовать SQL-запросы для получения данных из этой таблицы.
