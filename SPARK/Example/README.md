...
repartition(numPartitions) — полная перетасовка данных для изменения количества партиций. Всегда может увеличить или уменьшить число партиций, но требует дорогостоящего shuffle. Всегда вызывает полный shuffle (перемешивание данных по сети). Может как увеличивать, так и уменьшать количество партиций. Распределяет данные максимально равномерно. Ресурсоемкая операция — использует много сетевых и дисковых ресурсов. Изменяет схему партиционирования — "ломает" lineage (линию происхождения) в RDD.

coalesce(numPartitions) — оптимизированное уменьшение числа партиций. В основном используется для уменьшения без shuffle (или с минимальным shuffle). Не может увеличить число партиций. Обычно выполняется без shuffle (данные объединяются локально). Только уменьшает количество партиций. Не гарантирует равномерное распределение данных. Эффективнее repartition() при уменьшении партиций. Сохраняет существующее распределение там, где это возможно.
...
Делаем `mix.coalesce(3).rdd.glom().collect()` - будет сохранять в том виде в котором меньше всего перемешиваний. И не думает ровно, не ровно, перекос или еще что то.
