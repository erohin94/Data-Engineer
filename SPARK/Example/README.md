...
- [Работа с модулем functions](#Работа-с-модулем-functions)
- [Spark UI](#Spark-UI)
...
Например делаем `customs_data.cache()` и ничего не происходит, так как спрак ленивый и ничего не будет делать пока не скажем делай (actions). Поэтому делаю count `customs_data.cache().count()`.
На логи можно не обращать внимания. Их можно отключать и тд.

<img width="1273" height="716" alt="image" src="https://github.com/user-attachments/assets/4469c4fa-322e-40eb-b9c7-8f1a2c527e6d" />

Чтобы посмотреть как закешировал перехожу в UI Spark -> Storage.

<img width="1913" height="320" alt="image" src="https://github.com/user-attachments/assets/eb084f2f-b1ca-411b-8fb1-6e885d88ca21" />

Так как у меня не хватает памяти, то спарк не может записать весь датафрейм в оперативку, потому что спарк когда выделяет память, он выделяет определенное количество. Суть в том что у меня сейчас на драйвере не хватает, у меня нет 2ГБ. И он некоторую часть загрузил в память `On Heap Memory Usage`, а некоторую часть сбросил на диск `Disk Usage`.

<img width="1899" height="904" alt="image" src="https://github.com/user-attachments/assets/45a043a8-a6d5-4b41-8bd1-772e79af0f31" />

Здесь это уже RDD, это как раз тот формат который спарк держит внутри. И тут видно где то `Disk Serialized` - тоесть сжал и на диск загрузил. А где то `Memory Deserialized` - сжал и загрузил в память в десериализованном виде.

`Cached Partitions` у нас 16, это те самые партиции когда мы поделили в самом начале. Поскольку у меня один компьютер то `Executors` только один `4dab6efce3c1:36029`.

Чтобы раскешировать, чтобы убрать все это из UI Spark -> Storage. Делаю `customs_data.unpersist()`, но так же есть еще и другие варианты (см документацию).
Проверяю и вижу что в Storage ничего нет.

<img width="743" height="165" alt="image" src="https://github.com/user-attachments/assets/e284fe96-7bb0-48c7-b33f-75aa5951256b" />

**persist()**

Так же есть `persist()`, чем он отличается от `cache()`. Когда делаем `cache()` - он просто кеширует в памяти, а `persist()` мы можем сказать, сохрани только на диск, в памяти не надо. Или на диск и в память и тд. Тоесть вариант с более настройкой.
...
<img width="1908" height="289" alt="image" src="https://github.com/user-attachments/assets/b8e60acc-33e0-43d2-af42-b842f90775d0" />

<img width="1897" height="900" alt="image" src="https://github.com/user-attachments/assets/d4266556-2d17-48f2-aaca-cf564c882791" />

**Итог**

Если делаем: `df.cache()`. 
Например сделали все необходимые преобразования исходного датафрейма (группировку, фильтрацию, джоин и тд). 
Сохраняем этот датафрейм в переменную df и делаем кеширование. 
Spark сохранит преобразованный датафрейм в памяти и при последующих сохранениях не будет заново пересчитывать весь pipeline — фильтры, join’ы, groupBy.
...
cache() — кэширует DataFrame/RDD в памяти, повторный доступ не пересчитывает данные.
Кэширование происходит при первом действии (count, show).
Освобождение памяти: unpersist().

persist(level) — сохраняет DataFrame/RDD в памяти и/или на диске с разными уровнями хранения.
...
Например закончили работу с датафреймом и хотим сохранить результат, можно сохранить с помощью `df.write.csv("path")`, то будет не контролируемое сохранение
т.е Spark сохраняет каждую партицию в отдельный файл. Если у датафрейм был распределен по 16 executor, т.е 16 партиций — то будет 16 файликов:
Сколько партиций у датафрейма — столько файлов при сохранении. Это логика работы Spark: он пишет результат так же, как он его распределил внутри себя.
Если нужно НЕ 16 файлов, а меньше — используем инструменты управления партициями. Т.е контролируемое сохранение по количеству файлов

`coalesce(n)` - Объединяет существующие партиции без shuffle. `df.coalesce(1).write.csv("path")` Получится один файл.
Если написать `coalesce(4)`, Spark просто соберёт данные в 4 части, но размер файлов может быть неравномерный — Spark не перераспределяет данные. Плюсы: быстрый, без shuffle. Минусы: ненадёжно распределяет файлы по размеру.

`repartition(n)` - Полное перераспределение данных с shuffle. `df.repartition(4).write.csv("path")` Получится 4 файла, примерно одинакового размера. Плюсы: ровное распределение Минусы: shuffle — тяжёлая операция.
...
То есть делит по какой то своей логике и старается делить по ровну.

Дедаем `mix.coalesce(3).rdd.glom().collect()` - будет сохранять в том виде в котором меньше всего перемешиваний. И не думает ровно, не ровно, перекос или еще что то.
...
<img width="826" height="204" alt="image" src="https://github.com/user-attachments/assets/662b0ce0-0439-4ef1-94f7-be84948be71f" />

Не хватает памяти чтобы вывести весь датафрейм на драйвер, поэтому collect() лучше избегать и использовать show() или лимит. collect только на маленьких данных.

### Shuffle

Shuffle в Apache Spark — это процесс перераспределения данных между узлами кластера,
который происходит тогда, когда операция требует собрать строки с одинаковым ключом в одном разделе (partition).
Когда Spark выполняет действия вроде groupBy, join, reduceByKey, distinct, orderBy или repartition,
он вынужден передавать данные по сети между executors, записывать временные файлы на диск, сортировать ключи и загружать их обратно на нужные узлы,
чтобы продолжить вычисления. Из-за сетевого обмена, дисковой активности
и сортировки shuffle считается самым медленным и «дорогим» этапом выполнения задач Spark,
потому что он увеличивает задержку, потребляет много ресурсов и часто становится узким местом производительности.
Например, если выполнить операцию df.groupBy("department").agg({"salary": "avg"}),
Spark сначала локально сгруппирует данные по ключу department на каждом узле,
затем на этапе shuffle отправит части данных с одинаковыми ключами на одни и те же узлы по хэшу ключа,
а после этого на принимающих узлах выполнит финальную агрегацию.
Именно эта необходимость физически перетасовать данные между узлами и делает shuffle таким тяжёлым,
но при этом неизбежным механизмом в Spark.
