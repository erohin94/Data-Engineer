...
Далее переключаемся на нашу ветку `git checkout OPALAB-14848` (выполняем команду `git checkout -b OPALAB-14848` для создания новой ветки если ее нет. Команда `git checkout` без `-b` позволяет переключатся на существующую ветку)
...
**Добработки в ходе код ревью**

Проделываю все шаги заново, начиная с обновления локального репозитория. То есть так же переключаюсь на ветку мастер, обновляю, переключаюсь на рабочую ветку, обновляю скрипты, заливаю в свою рабочую ветку, отправляю в ББ.

Пулл реквест можно не делать изменения отображаются так.

<img width="1591" height="607" alt="image" src="https://github.com/user-attachments/assets/55a0ef98-7fe0-41a7-a846-b61905152372" />

![Снимок](https://github.com/user-attachments/assets/337a1c94-7d36-4569-ad13-8736bfb8df2f)
