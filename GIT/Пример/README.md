...
Возникла проблема, при отправки изменений в удаленный репозиторий, появились не понятные файлы и папки в Бит Бакете в моем пулл реквесте.

Решение: удлаить ветку локальную и на удалённом репозитории (например, в GitHub). После перезалить изменения заново.

Переключится на master ветку `git checkout master`. Нельзя удалить текущую ветку — поэтому сначала переключись на master. Работаю от нее.

Удалить ветку в удаленном репозитории `git push origin --delete OPALAB-14848`

Удалить локально `git branch -d OPALAB-14848`. Если Git ругается (например, ветка не смержена): `git branch -D OPALAB-14848`
...
Далее  как обычно: Делаем `git pull` - Скачивание изменений из удалённого репозитория в локальный и тд
